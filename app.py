...
async def scrape_leads(req: SearchRequest, request: Request):
    ...
    for item in results:
        ...
        raw_website = item.get("website", "Yok")

        # 🔒 Web sitesi ve e-posta bilgileri yalnızca premium kullanıcılara
        # gönderilir. Önceden website alanı premium olmayan kullanıcılara da
        # API yanıtında gidiyordu (sadece arayüzde CSS ile gizleniyordu),
        # bu da tarayıcı ağ sekmesinden görülebilmesine yol açıyordu.
        website = raw_website if is_premium else "Yok"
